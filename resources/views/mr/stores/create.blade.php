...
@section('page-title', 'Register New Store')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow">
        <div class="px-6 py-4 border-b">
            <h3 class="text-lg font-semibold">Store Registration Form</h3>
        </div>
        
        <form action="{{ route('mr.stores.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="store_name" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('store_name') border-red-500 @enderror" value="{{ old('store_name') }}">
                        @error('store_name')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Owner Name *</label>
                        <input type="text" name="owner_name" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('owner_name') border-red-500 @enderror" value="{{ old('owner_name') }}">
                        @error('owner_name')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Phone *</label>
                        <input type="text" name="phone" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('phone') border-red-500 @enderror" value="{{ old('phone') }}">
                        @error('phone')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                    </div>
                    
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Email</label>
                        <input type="email" name="email" class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('email') border-red-500 @enderror" value="{{ old('email') }}">
                ...
                <div class="mt-4">
                    ...
                    <textarea name="address" rows="3" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Locality / Area *</label>
                ...
                <div class="mt-4">
                    ...
                    <input type="text" name="pincode" id="pincode" maxlength="6" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 @error('pincode') border-red-500 @enderror" value="{{ old('pincode') }}">
                    @error('pincode')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>
            </div>
            
            <div class="flex justify-end space-x-4 pt-4 border-t">
                <a href="{{ route('mr.stores.index') }}" class="px-6 py-2 border rounded-lg hover:bg-gray-50">Cancel</a>
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">Register Store</button>
            </div>
        </form>
    </div>
</div>
...
function loadDistrictsAndSelect(stateId, districtId, districtName) {
    ...
    if (stateId) {
        fetch(`/mr/stores/get-districts/${stateId}`)
...
function loadCitiesAndSelect(districtId) {
    ...
    if (districtId && window.pendingTerritory) {
        fetch(`/mr/stores/get-cities/${districtId}`)
            .then(response => response.json())
            .then(data => {
                data.forEach(city => {
...
function loadAreasAndSelect(cityId) {
    ...
    if (cityId && window.pendingTerritory) {
        fetch(`/mr/stores/get-areas/${cityId}`)
            .then(response => response.json())
            .then(data => {
                data.forEach(area => {
