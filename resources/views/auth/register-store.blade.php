...
@section('title', 'Register as Store')
@section('page-title', 'Store Registration')

@section('content')
<div class="max-w-3xl mx-auto bg-white shadow rounded-lg p-6 sm:p-8">
    <h1 class="text-2xl font-bold text-gray-900 mb-2">Register as Store</h1>
    <p class="text-sm text-gray-600 mb-6">Your request will be submitted for admin approval.</p>

    <form method="POST" action="{{ route('store.register.store') }}" class="space-y-5">
        @csrf

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label class="block text-sm font-medium text-gray-700">Store Name *</label>
                <input type="text" name="store_name" value="{{ old('store_name') }}" required class="mt-1 w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                @error('store_name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700">Owner Name *</label>
                <input type="text" name="owner_name" value="{{ old('owner_name') }}" required class="mt-1 w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
                @error('owner_name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="phone" value="{{ old('phone') }}" required class="mt-1 w-full rounded-md border-gray-300 focus:border-indigo-500 focus:ring-indigo-500">
