...
@section('title', 'Add Feature – Platform Features')
...
<div class="p-6 max-w-3xl mx-auto space-y-6">
    ...
    <nav class="flex items-center gap-2 text-sm text-gray-500">
        ...
        <span>/</span>
        <span class="text-gray-900 font-medium">Add Feature</span>
    </nav>
    ...
    <div>
        ...
        <p class="text-sm text-gray-500 mt-1">Create a new feature card for the homepage Features section.</p>
    </div>

    {{-- Form --}}
    <form method="POST" action="{{ route('admin.homepage-features.store') }}">
        @csrf
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3"
                              class="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                              placeholder="Brief description of this feature...">{{ old('description') }}</textarea>
                ...
                <div>
                    ...
                    <div class="mt-2 flex items-center gap-2 text-gray-500 text-sm">
                        <span>Preview:</span>
                        <i class="fas fa-{{ old('icon', 'star') }} text-lg text-blue-600" id="icon-preview"></i>
                    </div>
                </div>
                        ...
                        @foreach($colors as $colorName => $classes)
                            <label class="cursor-pointer">
                                <input type="radio" name="icon_color" value="{{ $colorName }}" class="hidden peer"
                                       {{ old('icon_color') === $colorName ? 'checked' : '' }}>
