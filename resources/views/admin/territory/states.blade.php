...
@section('page-title', 'Territory Management / States')
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <table class="min-w-full">
            ...
            <tbody class="divide-y divide-gray-200">
                ...
                <tr>
                    <td class="px-6 py-4 font-mono text-sm">{{ $state->code }}</td>
                    <td class="px-6 py-4 font-medium">{{ $state->name }}</td>
                    <td class="px-6 py-4 text-sm">{{ $state->zone?->name ?? '-' }}</td>
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 text-xs rounded {{ $state->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                            {{ $state->is_active ? 'Active' : 'Inactive' }}
                        </span>
                    </td>
                    <td class="px-6 py-4 space-x-2">
                        <button onclick="editState({{ $state->id }}, '{{ $state->code }}', '{{ $state->name }}', {{ $state->zone_id ?? 'null' }}, {{ $state->is_active ? 1 : 0 }})" 
                            class="text-blue-600 hover:text-blue-800 text-sm">Edit</button>
                        <form action="{{ route('admin.territory.states.destroy', $state) }}" method="POST" class="inline" onsubmit="return confirm('Delete this state?');">
                            @csrf
                            @method('DELETE')
                ...
                @empty
                <tr>
                    <td colspan="5" class="px-6 py-8 text-center text-gray-500">No states found</td>
                </tr>
                @endforelse
            </tbody>
        </table>
...
<div id="createModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-md w-full p-6">
            <h2 class="text-xl font-bold mb-4">Add New State</h2>
            <form action="{{ route('admin.territory.states.store') }}" method="POST">
                @csrf
                <div class="space-y-4">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="zone_id" class="w-full border rounded-lg px-3 py-2">
...
<div id="editModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-md w-full p-6">
            <h2 class="text-xl font-bold mb-4">Edit State</h2>
            <form id="editForm" method="POST">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Zone</label>
                        <select name="zone_id" id="edit_zone_id" class="w-full border rounded-lg px-3 py-2">
                            <option value="">-- Select Zone --</option>
                            @foreach($zones as $zone)
                                <option value="{{ $zone->id }}">{{ $zone->name }}</option>
                            @endforeach
...
function editState(id, code, name, zoneId, isActive) {
    document.getElementById('editForm').action = `/admin/territory/states/${id}`;
